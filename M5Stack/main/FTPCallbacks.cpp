...
static std::string getFullFileName(std::string fileName) {
	std::string fullFileName = FTPServer::getCurrentDirectory();
...
void FTPFileCallbacks::onStoreStart(std::string fileName) {
	...
	m_storeFile.open(getFullFileName(fileName), std::ios::binary);                        // Open the file for writing.
	if (m_storeFile.fail()) {
...
void FTPFileCallbacks::onRetrieveStart(std::string fileName) {
	...
	m_retrieveFile.open(getFullFileName(fileName), std::ios::binary);
...
std::string FTPFileCallbacks::onDir() {
	printf("current: %s\n", FTPServer::getCurrentDirectory().c_str());
	...
	while(1) {
		...
		size_t fileSize = 0; // TODO: get file size properly;
		ss << ((pDirentry->d_type & DT_DIR) ? "d" : "-") << "rwxrwxrwx 1 ftp ftp " << fileSize;
		ss << " Jan 1 00:00 ";     
		ss << (std::string)(pDirentry->d_name) << "\n";
	}
	closedir(dir);
	return ss.str();
