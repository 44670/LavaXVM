...
#define FTP_CWD_MAX_PATH (512)
#define FTP_CWD_FORCE_HOME ("/sd")

static char ftpCurrentDir[FTP_CWD_MAX_PATH + 1] = {0};

static int ftpEnsureHome() {
	if (memcmp(FTP_CWD_FORCE_HOME, ftpCurrentDir, strlen(FTP_CWD_FORCE_HOME)) == 0) {
		char ch = ftpCurrentDir[strlen(FTP_CWD_FORCE_HOME)];
		if (ch == '/' || ch == 0) {
			// okay
			return 0;
		}
	}
	strcpy(ftpCurrentDir, FTP_CWD_FORCE_HOME);
	return -1;
}

const char* ftpGetCwd() {
	ftpEnsureHome();
	printf("cwd: %s\n", ftpCurrentDir);
	return ftpCurrentDir;
}

static int ftpWalkIntoDir(const char* dir) {
	...
	if (strcmp(dir, "..") == 0) {
		for (int i = strlen(ftpCurrentDir) - 1; i >= 0; i--) {
			if (ftpCurrentDir[i] == '/') {
				ftpCurrentDir[i] = 0;
				break;
			}
	...
	// TODO: check if the directory actually exists
	// TODO: fail if buffer is not enough
	if (strlen(ftpCurrentDir) > 1) {
		// don't add slash if currentdir is "/"
		strncat(ftpCurrentDir, "/", FTP_CWD_MAX_PATH - strlen(ftpCurrentDir));
	}
	strncat(ftpCurrentDir, dir, FTP_CWD_MAX_PATH - strlen(ftpCurrentDir));
	return 0;
} 

int ftpChdir(const char* path) {
	printf("chdir: %s\n", path);
	char workBuf[FTP_CWD_MAX_PATH + 1];
	if (strlen(path) <= 0) {
		return -1;
	}
	if (strlen(path) > FTP_CWD_MAX_PATH) {
		return -1;
	}
	static char prevCurrentDir[FTP_CWD_MAX_PATH + 1];
	strcpy(prevCurrentDir, ftpGetCwd());
	if (path[0] == '/') {
		strcpy(ftpCurrentDir, "/");
	}
	int pathLen = strlen(path);
	int prevSeparatorAt = 0;
	if (path[0] != '/') {
		prevSeparatorAt = -1;
	}
	for (int i = 1; i < pathLen + 1; i++) {
		char ch = path[i];
		if ((ch == '/') || (ch == 0)) {
			// check and skip repeated separators
			if (prevSeparatorAt + 1 < i) {
				memcpy(workBuf, path + prevSeparatorAt + 1, i - prevSeparatorAt - 1);
				workBuf[i - prevSeparatorAt - 1] = 0;
				if (ftpWalkIntoDir(workBuf) != 0) {
					goto fail;
				}
			}
			prevSeparatorAt = i;
	...
	return 0;
fail:
	strcpy(ftpCurrentDir, prevCurrentDir);
	return -1;
}
...
/* STATIC */ std::string FTPServer::getCurrentDirectory() {
	...
	std::string currentDirectory = ftpGetCwd();
...
void FTPServer::receiveFile(std::string fileName) {
	...
	while(1) {
		...
		if (m_callbacks != nullptr) {
			m_callbacks->onStoreData(buf, rc);
		}
		totalSizeRead += rc;
	}
	sendResponse(FTPServer::RESPONSE_226_CLOSING_DATA_CONNECTION); // Closing data connection.
