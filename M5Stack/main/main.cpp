...
void FileChange(char *fname,char *ram,long len)
{
	...
	fp=platFopen(fname,"wb");
	if (fp!=NULL) {
		platFwrite(fp,(u8*) ram,len);
		platFclose(fp);
	}
}
...
int UnPac(char *pacname,char *zname)
{
	...
	have_lav=0;
	if ((fp=platFopen(pacname,"rb"))==NULL) {
		FATAL("无法打开PAC文件!");
		return 0;
	}
	platFseek(fp,0,PLAT_SEEK_SET);
	...
	for (i=0;i<total;i++) {
		...
		if (filelen) {
			...
			p=(char*)pLAVA; 
			if (filelen >= PLAVA_SIZE) {
				platFclose(fp);
				return 0;
			}
			platFread(fp,(u8*) p,filelen);
...
u8 translateKey(u8 keyval) {
	return keyboardKeyMap[keyval];
}

void updateKey() {

	if ((lav_key & 0x80)) {
		// there is an unhandled key
		return;
	}
...
void app_main()
{
    ...
    esp_err_t ret = spi_flash_mmap(0x00200000, 0x00100000, SPI_FLASH_MMAP_DATA,
        (const void**) &mappedResourceData, &resourceDataMapHandle);
    if (ESP_OK != ret) {
